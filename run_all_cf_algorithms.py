...
from Data_manager.Books.BooksReader import BooksReader
...
def _get_instance(recommender_class, URM_train, ICM_all, UCM_all):
    ...
    if issubclass(recommender_class, BaseItemCBFRecommender):
        print(f"WRONG MODEL: {recommender_class} IS NOT CF" )
...
if __name__ == '__main__':
    ...
    dataset_object = BooksReader()

    dataSplitter = DataSplitter_leave_k_out(dataset_object, k_out_value=2)

    dataSplitter.load_data(save_folder_path=False)
    ...
    evaluator = EvaluatorHoldout(URM_test, cutoff_list, exclude_seen=True)

    # from MatrixFactorization.PyTorch.MF_MSE_PyTorch import MF_MSE_PyTorch

    earlystopping_keywargs = {"validation_every_n": 5,
                              "stop_on_validation": True,
                              "evaluator_object": EvaluatorHoldout(URM_validation, cutoff_list, exclude_seen=True),
    ...
    logFile = open(output_root_path + "result_all_cf_algorithms.txt", "a")
