...
class EASE_R_RecommenderV2(BaseItemSimilarityMatrixRecommender):
    """ EASE_R_Recommender
    ...
    def fit(self, topK=None, l2_norm = 1e3, normalize_matrix = False):
        ...
        grahm_matrix = self.URM_train.T.dot(self.URM_train)

        diag_indices = np.diag_indices(grahm_matrix.shape[0])
        grahm_matrix[diag_indices] += l2_norm

        # P = spsl.inv(grahm_matrix)  # safe but slow
        P = sps.csr_matrix(scipy.linalg.inv(grahm_matrix.todense())) # 8GB ram required faster
